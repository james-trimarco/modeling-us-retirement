...
### Theme for plots
Mix of Roboto with a little Spectral mixed in. 
```{r create_theme}
...
Later on in this analysis, we'll see that statistically signicant differences in the probability of retirement depend on sex and whether a person was born in the U.S. The plots below are intended to put those conclusions into context.

#### Boxplot by sex & birth country
Note that this plot uses the unweighted data. A plot using the `svyboxplot()` function looks nearly identical. 
...
Clearly, there's a relationship between age and the probability of retirement: We expect the latter to increase along with age. But a model that includes only age is going to miss subtler relationships like the one with sex that we just plotted. How well can we predict the probability of retirement using age alone?
...
#### View coefficients
```{r summ_simple}
# This library gives us a few key functions for summarizing logistic models 
...
When we set the cutoff at 0.5, we guess the retirement status of the non-retired majority of cases correctly 96.0% of the time. But for the minority of cases that are retired, we're guessing right only 61.1% of the time. 
```{r}
cutoff <- 0.5
...
I'm using 99% confidence here because our n is so large that the confidence bands are right up against the prediction line if we use 95% confidence. 
...
### Model with age & sex  
We already showed above that sex also helps explain whether we can expect a given person to be retired or not. How does the model change if we add sex and an interaction between sex and age?
...
The results are not ambiguous. Age, sex, and the interaction between them are all highly statistically significant. This means that the effect of age on the response is different for men and women.
...
#### Sensitivity and specificity
When we set the cutoff at 0.5, we guess the retirement status of the non-retired majority of cases correctly 95.8% of the time — slightly worse than the simple model. But for the minority of cases that are retired, we're guessing right only 63.2% of the time. That's a significant improvement over the simple model. 
